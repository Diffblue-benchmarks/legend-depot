...
class ManageQueryMetricsSchedulesModuleDiffblueTest {
  /**
   * Test {@link ManageQueryMetricsSchedulesModule#scheduleMetricsConsolidation(SchedulesFactory,
   * QueryMetricsService)}.
   *
   * <p>Method under test: {@link
   * ManageQueryMetricsSchedulesModule#scheduleMetricsConsolidation(SchedulesFactory,
   * QueryMetricsService)}
   */
  @Test
  ...
  void testScheduleMetricsConsolidation() {
    // Arrange
    ManageQueryMetricsSchedulesModule manageQueryMetricsSchedulesModule =
    ...
    // Act
    boolean actualScheduleMetricsConsolidationResult =
        manageQueryMetricsSchedulesModule.scheduleMetricsConsolidation(
            schedulesFactory, new QueryMetricsServiceImpl(new QueryMetricsMongo(databaseProvider)));

    // Assert
    verify(schedulesFactory)
        .registerSingleInstance(
            eq("consolidate-query-metrics"), eq(6000L), eq(21600000L), isA(Supplier.class));
