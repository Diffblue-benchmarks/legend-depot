...
class ManageProjectsResourceDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Given {@link ManageProjectsServiceImpl} {@link ManageProjectsServiceImpl#delete(String,
   *       String)} return one.
   *   <li>Then return one.
   * </ul>
   *
   * <p>Method under test: {@link ManageProjectsResource#deleteProject(String, String)}
   */
  @Test
  @DisplayName(
      "Test deleteProject(String, String); given ManageProjectsServiceImpl delete(String, String) return one; then return one")
  ...
  void testDeleteProject_givenManageProjectsServiceImplDeleteReturnOne_thenReturnOne() {
    ...
    ManageProjectsResource manageProjectsResource =
        new ManageProjectsResource(projectApi, authorisationProvider, mock(Provider.class));

    // Act
    long actualDeleteProjectResult = manageProjectsResource.deleteProject("42", "42");

    // Assert
    verify(authorisationProvider).authorise(isA(Provider.class), eq("Projects"));
    verify(projectApi).delete("42", "42");
