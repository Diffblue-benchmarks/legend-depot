...
class ManageNotificationsQueueMongoModuleDiffblueTest {
  /**
   * Test {@link ManageNotificationsQueueMongoModule#registerIndexes(MongoAdminStore)}.
   * <p>
   * Method under test: {@link ManageNotificationsQueueMongoModule#registerIndexes(MongoAdminStore)}
   */
  @Test
  ...
  void testRegisterIndexes() {
    // Arrange
    ManageNotificationsQueueMongoModule manageNotificationsQueueMongoModule = new ManageNotificationsQueueMongoModule();
    CodecRegistry codecRegistry = mock(CodecRegistry.class);
    ReadPreference readPreference = mock(ReadPreference.class);
    WriteConcern writeConcern = new WriteConcern(1);

    // Act and Assert
    assertTrue(manageNotificationsQueueMongoModule.registerIndexes(
  ...
  void testRegisterIndexes_thenCallsRegisterIndexes() {
    ...
    // Act
    boolean actualRegisterIndexesResult = manageNotificationsQueueMongoModule.registerIndexes(adminStore);

    // Assert
    verify(adminStore).registerIndexes(eq("notifications-queue"), isA(List.class));
