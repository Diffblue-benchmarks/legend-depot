...
public class TestArtifactsPurgeService extends TestBaseServices
{
    ...
    public void canDeleteSnapshotVersions()
    {
        ...
        String result = purgeService.deleteSnapshotVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, Arrays.asList(snapshot, snapshot2));
    ...
    public void canDeleteSnapshotVersionsButExceptionOnNonSnapShotVersions()
    {
        ...
        String message = "Version 2.0.0 can't be deleted as it is not a snapshot version\nVersion 2.2.0 can't be deleted as it is not a snapshot version\nDeleted remaining versions";
        List<String> allVersions;
        allVersions = projectsService.getVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, true);
        ...
        String result = purgeService.deleteSnapshotVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, Arrays.asList(snapshot, snapshot2, versionId, versionId2));
    ...
    public void canDeleteSnapshotVersionsButExceptionOnDefaultBranch()
    {
        ...
        String message = "Version master-SNAPSHOT can't be deleted as it is the project's default branch\nDeleted remaining versions";
        List<String> allVersions;
        allVersions = projectsService.getVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, true);
        Assertions.assertTrue(allVersions.contains(defaultSnapShot));

        String result = purgeService.deleteSnapshotVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, Arrays.asList(defaultSnapShot, snapshot, snapshot2));
        Assertions.assertEquals(message, result);
        allVersions = projectsService.getVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, true);
        Assertions.assertEquals(4, allVersions.size());
        Assertions.assertFalse(allVersions.contains(snapshot));
    ...
    public void canDeleteSnapshotVersionsButExceptionOnNonSnapShotVersionAndDefaultBranch()
    {
        ...
        String versionId = "2.0.0";
        String versionId2 = "2.2.0";
        String message = "Version master-SNAPSHOT can't be deleted as it is the project's default branch\nVersion 2.0.0 can't be deleted as it is not a snapshot version\nVersion 2.2.0 can't be deleted as it is not a snapshot version\nDeleted remaining versions";

        String result = purgeService.deleteSnapshotVersions(TEST_GROUP_ID, TEST_ARTIFACT_ID, Arrays.asList(defaultSnapShot, snapshot, snapshot2, versionId, versionId2));
        Assertions.assertEquals(message, result);
