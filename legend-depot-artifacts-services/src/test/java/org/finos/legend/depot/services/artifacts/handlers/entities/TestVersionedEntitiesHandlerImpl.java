...
public class TestVersionedEntitiesHandlerImpl
{
    ...
    private final ManageVersionedEntitiesService versionedEntitiesService = mock(ManageVersionedEntitiesService.class);
    private final VersionedEntityArtifactsProvider artifactProvider = mock(VersionedEntityArtifactsProvider.class, Mockito.withSettings().extraInterfaces(EntityArtifactsProvider.class));
    ...
    {
        when(versionedEntitiesService.delete(TEST_GROUP_ID, TEST_ARTIFACT_ID, TEST_VERSION_ID)).thenReturn(5L);
        VersionedEntitiesHandlerImpl handler = new VersionedEntitiesHandlerImpl(versionedEntitiesService, artifactProvider);
        handler.delete(TEST_GROUP_ID, TEST_ARTIFACT_ID, TEST_VERSION_ID);
        verify(versionedEntitiesService).delete(TEST_GROUP_ID, TEST_ARTIFACT_ID, TEST_VERSION_ID);
    }
}
