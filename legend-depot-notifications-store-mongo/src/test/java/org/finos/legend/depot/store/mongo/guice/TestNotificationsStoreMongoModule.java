...
{
    private NotificationsStoreMongoModule module = new NotificationsStoreMongoModule();
    ...
    @Test
    public void registerIndexesCreatesCorrectIndexes()
    {
        MongoAdminStore adminStore = new MongoAdminStore(mongoProvider);

        module.registerIndexes(adminStore);
        List<String> createdIndexes = adminStore.createIndexes();

        Assertions.assertNotNull(createdIndexes);
        List<IndexModel> expectedIndexes = NotificationsMongo.buildIndexes();
        Assertions.assertEquals(expectedIndexes.size(), createdIndexes.size());
    }
}
