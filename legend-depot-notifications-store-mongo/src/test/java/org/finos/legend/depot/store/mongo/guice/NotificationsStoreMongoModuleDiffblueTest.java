...
class NotificationsStoreMongoModuleDiffblueTest {
  /**
   * Test {@link NotificationsStoreMongoModule#registerIndexes(MongoAdminStore)}.
   * <p>
   * Method under test: {@link NotificationsStoreMongoModule#registerIndexes(MongoAdminStore)}
   */
  @Test
  ...
  void testRegisterIndexes() {
    // Arrange
    NotificationsStoreMongoModule notificationsStoreMongoModule = new NotificationsStoreMongoModule();
    CodecRegistry codecRegistry = mock(CodecRegistry.class);
    ReadPreference readPreference = mock(ReadPreference.class);
    WriteConcern writeConcern = new WriteConcern(1);

    // Act and Assert
    assertTrue(notificationsStoreMongoModule.registerIndexes(
  ...
  void testRegisterIndexes_thenCallsRegisterIndexes() {
    ...
    // Act
    boolean actualRegisterIndexesResult = notificationsStoreMongoModule.registerIndexes(adminStore);

    // Assert
    verify(adminStore).registerIndexes(eq("notifications"), isA(List.class));
